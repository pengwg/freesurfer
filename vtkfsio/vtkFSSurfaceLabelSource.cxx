...
void vtkFSSurfaceLabelSource::AddVerticesToLabel(int icVertices,
                                                 int *iaVertices) {
  assert(icVertices > 0);
  assert(iaVertices);
  assert(Mris);
  ...
  for (int nVertex = 0; nVertex < icVertices; nVertex++) {
    ...
    if (iaVertices[nVertex] < 0 || iaVertices[nVertex] >= Mris->nvertices)
...
void vtkFSSurfaceLabelSource::RemoveVerticesFromLabel(int icVertices,
                                                      int *iaVertices) {
...
void vtkFSSurfaceLabelSource::Execute() {

  if (NULL == Mris)
    vtkErrorMacro(
        << "vtkFSSurfaceLabelSource cannot exectue without a surface");

  if (NULL == Label) {
    this->ReadLabelFile();
    if (NULL == Label)
      vtkErrorMacro(
          << "vtkFSSurfaceLabelSource cannot exectue without a label");
  }
  ...
  for (int nFace = 0; nFace < Mris->nfaces; nFace++) {
    if (Mris->vertices[Mris->faces[nFace].v[0]].marked &&
      ...
      // surface.
      for (int nFaceVertex = 0; nFaceVertex < VERTICES_PER_FACE;
           nFaceVertex++) {
...
void vtkFSSurfaceLabelSource::ReadLabelFile() {

  if (NULL == LabelFileName)
    vtkErrorMacro(
        << "vtkFSSurfaceLabelSource cannot exectue without a label file name");

  // Load the white vertex positions in the surface.
  int eMRIS = MRISreadWhiteCoordinates(Mris, (char *)"white");
  if (eMRIS != 0)
    throw runtime_error(
        "Couldn't read the white surface file, so unable to load a label.");
...
void vtkFSSurfaceLabelSource::WriteLabelFile() {

  assert(Label);
  if (NULL == LabelFileName)
    vtkErrorMacro(
        << "vtkFSSurfaceLabelSource cannot write without a label file name");
...
void vtkFSSurfaceLabelSource::GetLabeledPoints(vtkPoints &ioPoints) {
...
void vtkFSSurfaceLabelSource::GetLabeledVertices(vector<int> &iolVertices) {
